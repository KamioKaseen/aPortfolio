          ...
          {bodyText.reduce((acc: ContentItem[], { text, textItem, title }, index) => {  
            ...
            if (textItem) {  
              ...
              if (prev && prev.type === "list") {  
                prev.list.push(<li key={index} className={styles.content__listItem}>{textItem}</li>);  
              } else {  
                // Создаем новый список и добавляем в него первый элемент  
                acc.push({ type: "list", list: [<li key={index} className={styles.content__listItem}>{textItem}</li>] });  
              }  
            } else {  
              ...
              if (title) {  
                const paragraphElement = (  
                  <div className={styles.group}>  
                    ...
                    <p key={index} className={styles.content__text}>{text}</p>  
                  </div>  
                );  
                acc.push({ type: "text", element: paragraphElement });  
              } else {  
                // Если нет заголовка, просто добавляем параграф  
                const paragraphElement = <p key={index} className={styles.content__text}>{text}</p>;  
                acc.push({ type: "text", element: paragraphElement });  
              }  
            }  
            ...
            item.type === "text" ? item.element : <ul key={i} className={styles.content__list}>{item.list}</ul>  
          )}  
        </>  
        }  
